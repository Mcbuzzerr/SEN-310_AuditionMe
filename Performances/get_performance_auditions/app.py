...
def lambda_handler(event, context):
    ...
    if "mine" == path["id"]:
        ...
        if user_role == "Director":
            ...
            performances.append(table.scan(FilterExpression=Attr("auditions").contains(event['requestContext']['authorizer']['userId'])["Items"]))
    else:
        id = path["id"]
    ...
    for performance in performances:
        fetched_auditions = []
        for audition in performance["auditions"]:
            performer = boto3.resource('dynamodb', region_name=region_name ).Table('AuditionMePeople').get_item(Key={"Id":audition})["Item"]
            del performer["password"]
            fetched_auditions.append(performer)
        performance["auditions"] = fetched_auditions
